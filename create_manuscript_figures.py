...
def create_time_plot(group_var, inst_var):
    ...
    fig.update_yaxes(title='Cells',
                     ...
                     col=1,
                     type='log')
    fig.write_image("current_figures/time_graph.eps",
                    width=800,
                    height=500,
                    engine="kaleido"
                    )
...
def create_mutation_profile(data_driver_sim):
    ...
    plt.savefig("current_figures/mutational_dinamics.svg")
    # plt.show()
...
def create_param_comparison(driv_data, bcr_data):
    ...
    fig.savefig("current_figures/new_relationship.svg", format="svg", transparent=True)


if __name__ == "__main__":
    os.mkdir("current_figures")
    create_time_plot(3180, 80)
    driv_data = import_sum_dataset("simulated_data/filtered_full_final_driver.csv")
    only_bcr_data = import_sum_dataset("simulated_data/filtered_full_final_no_mut.csv")
    ...
    image.write_image("current_figures/median_durations.eps", engine="kaleido")
